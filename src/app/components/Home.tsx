    ...
    >
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 pt-24 md:pt-32 pb-12 md:pb-24">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-24 mb-10 lg:mb-20">
          ...
          {/* Text Content */}
          <div className="w-full lg:w-1/2 space-y-8 md:space-y-12 text-center lg:text-left order-1 lg:order-1">
            
            <div className="flex justify-center lg:justify-start">
              <div className="flex items-center gap-3 sm:gap-6 px-6 py-4 rounded-full border border-white/10 bg-white/5 backdrop-blur-2xl shadow-2xl transition-all duration-500 cursor-default select-none group max-w-full overflow-x-auto no-scrollbar">
                <StatusItem icon={<Popcorn className="w-4 h-4 sm:w-5 sm:h-5" />} label="EAT" hoverColor="hover:text-cyan-400" />
                <span className="text-gray-800">•</span>
                <StatusItem icon={<Code2 className="w-4 h-4 sm:w-5 sm:h-5" />} label="CODE" hoverColor="hover:text-purple-400" />
                <span className="text-gray-800">•</span>
                <StatusItem icon={<Bot className="w-4 h-4 sm:w-5 sm:h-5" />} label="BUILD" hoverColor="hover:text-yellow-400" />
                <span className="text-gray-800 hidden sm:inline">•</span>
                <div className="hidden sm:flex items-center gap-2.5 hover:text-green-400 transition-colors duration-300">
                   <RefreshCw className="w-5 h-5 animate-spin-slow" />
                   <span className="text-[10px] sm:text-xs font-black tracking-[0.2em] text-gray-500">REPEAT</span>
                </div>
              </div>
            </div>
            ...
            <p className="text-lg sm:text-xl md:text-2xl text-gray-500 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-light px-4 lg:px-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center lg:justify-start w-full sm:w-auto px-6 sm:px-0">
              <button onClick={() => onNavigate('Projects')} className="w-full sm:w-auto group px-8 py-4 sm:px-10 sm:py-5 bg-white text-black rounded-2xl font-bold text-base sm:text-lg hover:bg-cyan-400 transition-all flex items-center justify-center gap-3 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                Explore Projects <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </button>
              <button onClick={() => onNavigate('Contact')} className="w-full sm:w-auto px-8 py-4 sm:px-10 sm:py-5 border border-white/10 bg-white/5 rounded-2xl font-bold text-base sm:text-lg hover:bg-white/10 transition-all">
                Contact Me
              </button>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 flex justify-center lg:justify-end relative order-2 lg:order-2 mt-8 lg:mt-0">
            <div className="relative w-[280px] sm:w-[400px] lg:w-full max-w-xl aspect-square group">
              <AnimatePresence>
                {showSpeechBubble && (
                  <motion.div 
                    ...
                    className="absolute top-[-15%] sm:top-[-10%] left-1/2 -translate-x-1/2 z-50 pointer-events-none w-max max-w-[90vw]"
                  >
                    <div className="bg-white text-black px-4 py-2 sm:px-6 sm:py-3 rounded-2xl shadow-[0_10px_40px_rgba(34,211,238,0.4)] relative border-2 border-cyan-400">
              ...
              <div className="relative h-full w-full rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-white/10 bg-black/40 backdrop-blur-md shadow-3xl">
                <ExpressiveRobot action={robotState} />
              </div>

              <button 
                onClick={() => setRobotState('Dance')}
                className="absolute -bottom-4 -left-4 sm:-bottom-6 sm:-left-6 w-16 h-16 sm:w-20 sm:h-20 bg-[#111] border border-cyan-500/30 rounded-2xl p-3 sm:p-4 shadow-2xl hover:scale-110 active:scale-95 transition-all flex items-center justify-center group/btn z-40"
              >
                <Bot className="w-8 h-8 sm:w-10 sm:h-10 text-cyan-500 group-hover/btn:animate-bounce" />
        ...
        {/* --- FLOATING SKILLS SECTION (UPDATED) --- */}
        <div className="py-12 md:py-24 border-t border-white/5 relative z-20">
            <div className="text-center mb-4 md:mb-8">
                <h2 className="text-3xl md:text-5xl font-black tracking-tighter mb-4 flex items-center justify-center gap-3">
                    <Command className="w-8 h-8 md:w-12 md:h-12 text-gray-500" />
            ...
            {/* The Floating Skills Canvas */}
            <div className="h-[400px] md:h-[500px] w-full -mt-4 md:-mt-10 cursor-grab active:cursor-grabbing">
                <FloatingSkills />
            </div>
        </div>
        ...
        <div className="pt-12 md:pt-24 border-t border-white/5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
            <ActivityCard 
              ...
              label="Achievement" 
              title="1st Place Techmela" 
              desc="Won top prize for designing a neural-link waste segregation hardware unit." 
            ...
            <ActivityCard 
              ...
              desc="AI-driven segregation using TensorFlow Lite on ESP32 to classify and sort waste." 
            ...
            <ActivityCard 
              ...
              label="Certification" 
              title="MATLAB Onramp" 
              desc="Course completion certificate for mastering MATLAB fundamentals and applications." 
              btnText="More" 
...
function StatusItem({ icon, label, hoverColor }: { icon: React.ReactNode, label: string, hoverColor: string }) {
  return (
    <div className={`flex items-center gap-1.5 sm:gap-2.5 text-gray-500 transition-colors duration-300 ${hoverColor} cursor-default whitespace-nowrap`}>
      {icon}
...
function ActivityCard({ icon, label, title, desc, btnText, onClick }: any) {
  return (
    <div onClick={onClick} className="group bg-[#111] border border-white/5 rounded-[2rem] p-8 sm:p-10 transition-all cursor-pointer hover:-translate-y-3 shadow-2xl hover:border-white/20 active:scale-[0.98]">
      <div className="mb-6 sm:mb-8 p-4 w-fit rounded-2xl bg-white/5 border border-white/5 group-hover:scale-110 transition-transform duration-500">
        {icon}
      </div>
